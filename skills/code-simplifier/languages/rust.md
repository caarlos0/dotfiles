...
- Hunt for dead code (orphaned by the change, or already stale in these files): the compiler's `dead_code`/`unused` warnings and `cargo clippy` flag unused items. Delete it — don't silence it with `#[allow(dead_code)]`.
