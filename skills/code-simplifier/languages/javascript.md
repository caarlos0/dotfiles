...
- Hunt for dead code (orphaned by the change, or already stale in these files): eslint's `no-unused-vars`/`no-unreachable` flag locals and dead branches; `knip` finds unused exports and files. Delete what's truly dead.
